...
export default function PaymentList({ payments, projects, services, onRefresh, loading }) {
  ...
  return (
    <div style={{ padding: '0 16px 24px' }}>
      ...
      {/* 추가 폼 (상단 고정) */}
      {showAddForm && (
        <InlineForm form={form} setForm={setForm} onSave={save} onClose={closeForm} saving={saving} isEdit={false} projects={projects} services={services} />
      )}
        ...
        if (isEditing) {
          return (
            <div key={p.id} style={{ marginBottom: 10 }}>
              <InlineForm form={form} setForm={setForm} onSave={save} onClose={closeForm} saving={saving} isEdit={true} projects={projects} services={services} />
            </div>
          )
        }
...
function InlineForm({ form, setForm, onSave, onClose, saving, isEdit, projects, services }) {
  return (
